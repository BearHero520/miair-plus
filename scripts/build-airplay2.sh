...
cd ffmpeg
./configure --prefix=/opt/miair-audio --disable-everything --disable-autodetect --disable-doc --disable-debug --disable-ffplay --enable-small --enable-shared --disable-static --enable-gpl --enable-version3 --enable-openssl --enable-libmp3lame --enable-ffmpeg --enable-ffprobe --enable-avcodec --enable-avformat --enable-avutil --enable-swresample --enable-avfilter --enable-protocol=file,pipe,http,https,tcp,tls,crypto --enable-demuxer=mov,mp3,aac,flac,wav,ogg --enable-muxer=mp3,mp4,adts,wav,s16le --enable-parser=aac,mpegaudio,flac,opus,vorbis --enable-decoder=alac,aac,mp3,flac,pcm_s16le,pcm_s16be,pcm_s24le,pcm_s32le,vorbis,opus --enable-encoder=libmp3lame,pcm_s16le,alac --enable-filter=aresample,aformat,anull,volume,asetpts,atrim,sine --enable-indev=lavfi --enable-bsf=aac_adtstoasc
make -j"$(nproc)"
make install
