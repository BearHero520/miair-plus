...
支持 MP3、WAV、FLAC、OGG、AAC，单文件最大 100 MB。保存时提前准备铃声。播放方式可选“整首播放一次”、“循环播放整首”（2–20 次，包含第一遍）或“按时长播放”（播放一次，播完或达到 1–10 分钟上限后结束）。整首与循环模式不按分钟截断；停止或延后会终止本次播放。新闹钟默认整首播放，已有闹钟保留原来的时长设置。长音频与多次循环准备时间较长。
...
- **自动检测更新**：默认在打开管理界面时及使用期间每小时检测 GitHub 正式版本，有新版本时显示提示。“设置”和“关于”提供检测开关、手动检测、更新说明与下载入口；仅提示更新，不自动安装。正常结果缓存一小时，手动检测与失败重试至少间隔一分钟。
- **配置导入与导出**：在“设置”下载 JSON 备份，包含已保存的应用设置、音箱偏好与闹钟规则，不含账号登录凭据、音乐文件及浏览器外观。导入支持最大 1 MB 文件，确认后整体校验并保存；格式错误时保留原配置。恢复全部闹钟时会先关闭它们，请检查音箱、重新选择铃声并保存后启用；未连接的音箱偏好会跳过。正在响铃或处理铃声时暂不允许导入。
...
| AirPlay 2 | 预览功能，单个接收实例选择一个目标音箱；需要系统 D-Bus、Avahi 及可用的 TCP 接收端口（默认 7000，可在设置中自定义）及 UDP 319/320 端口 |
| 音频投送范围 | 不支持屏幕镜像、视频接收，不保证多房间精确同步 |

当前版本为 2.0.9，新增闹钟整首播放与按次数循环，修复移动端输入字号、AirPlay 音量请求阻塞及音箱音量同步；已通过模拟会话、真实 FFmpeg 与页面检查，NAS 实机投送仍待验证。2.0.8 修复内置 Shairport Sync 覆盖自定义端口的问题，支持在设置中自定义 AirPlay 2 TCP 端口（默认 7000）。端口保存、配置迁移与备份恢复已通过自动化测试，NAS 自定义端口投送仍需实机验收。2.0.6 新增自动检测更新、配置导入导出，并修复闹钟时间与重复规则的对齐。已通过自动化测试与桌面、手机宽度的页面检查，新增功能仍需在 NAS 上安装验收。此前 2.0.5 的投送与飞牛远程入口已获用户实测确认，其他音箱型号与发送端的兼容性仍需分别验证。详细状态见 [兼容与测试说明](docs/COMPATIBILITY.md)。
