...
当前版本为 2.0.6，新增自动检测更新、配置导入导出，并修复闹钟时间与重复规则的对齐。已通过自动化测试与桌面、手机宽度的页面检查，新增功能仍需在 NAS 上安装验收。此前 2.0.5 的投送与飞牛远程入口已获用户实测确认，其他音箱型号与发送端的兼容性仍需分别验证。详细状态见 [兼容与测试说明](docs/COMPATIBILITY.md)。
