<template>
 <div class="app-workspace">
  ...
  <aside class="app-sidebar glass">
   ...
   <div class="sidebar-bottom"><span class="sidebar-label">MiAir Plus</span><small>2.0.0-alpha.6</small></div>
  </aside>
...
import {AlarmOutline,GridOutline,VolumeHighOutline,PersonOutline,SettingsOutline,DocumentTextOutline,SunnyOutline,MoonOutline,RadioOutline} from '@vicons/ionicons5'
import {useAppStore} from '@/stores/app'
import HeaderBar from './components/HeaderBar.vue'
const app=useAppStore(),route=useRoute()
const navigation=[{name:'dashboard',label:'总览',icon:GridOutline},{name:'devices',label:'音箱',icon:VolumeHighOutline},{name:'alarms',label:'闹钟',icon:AlarmOutline},{name:'account',label:'账号',icon:PersonOutline},{name:'settings',label:'设置',icon:SettingsOutline},{name:'logs',label:'日志',icon:DocumentTextOutline}]
const descriptions:Record<string,string>={alarms:'为日常与假日分别设置提醒',devices:'管理音箱与投送名称',account:'连接小米账号，选择音箱',settings:'投送、连接与外观',logs:'查看投送记录，定位连接与播放问题'}
</script>
