<template>
 ...
 <div class="alarm-grid">
  <section v-for="row in rows" :key="row.alarm.id" class="glass alarm-card">
   ...
   <p class="alarm-sound">{{row.alarm.sound}}</p>
 ...
 <n-modal v-model:show="show" preset="card" :title="form.id?'编辑闹钟':'新建闹钟'" class="alarm-modal" :mask-closable="!busy" :closable="!busy">
  <n-form label-placement="top">
   ...
   <n-form-item label="NAS 铃声"><div class="alarm-file-choice"><n-input :value="form.sound" readonly placeholder="从 NAS 铃声目录选择"/><n-button @click="openFiles('.')">选择文件</n-button></div></n-form-item>
   ...
   <p v-if="busy">正在保存并准备铃声，请稍候…</p><n-button type="primary" block :loading="busy" @click="save">保存闹钟</n-button>
  </n-form>
 </n-modal>
...
async function save(){busy.value=true;saveError.value='';try{await http.post('/alarms',form,{timeout:60000});show.value=false;await load();message.success('闹钟已保存')}catch(e){saveError.value=reason(e)}finally{busy.value=false}}
